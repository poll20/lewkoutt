...
export default function Filter() {
  const { wearsdata, filters, setFilters, sortOption, setSortOption } = useBio();
  const [selectedFilter, setSelectedFilter] = useState('pricerange');
  ...
  const handleDiscountChange = (discount) => {
    setFilters((prev) => {
      ...
      if (updatedDiscount.includes(discount)) {
        updatedDiscount.splice(updatedDiscount.indexOf(discount), 1);
      } else {
        updatedDiscount.push(discount);
      }
      return { ...prev, discount: updatedDiscount };
    });
  };

  const distinctcat = [...new Set(wearsdata.map((cat) => cat.category))];

  const handleSubMenu = (category) => {
    setSubMenu(category === subMenu ? null : category);
    const taging = wearsdata.filter((tags) => tags.category === category);
    const alltags = taging.map((e) => e.tag);
    const distincttag = [...new Set(alltags)];
    settags(distincttag);
  };
  ...
  const { pricerangemin, pricerangemax, sizes, color, categories, occasion, nackline, clothingmaterial, styletype, printtype, discount } = filters;
  ...
  }, [pricerangemin, pricerangemax, sizes, color, categories, occasion, nackline, clothingmaterial, styletype, printtype, discount]);
      ...
      <div className="myfilter-container">
        <div className="myfilter-name">
          ...
          <div style={{ marginBottom: "15px" }}>
            <label style={{ fontWeight: "bold", color: "green" }}>Sort By:</label>
            <select
              value={sortOption}
          ...
          >
            <div
              className={`filter-item ${selectedFilter === "pricerange" ? "active" : ""}`}
              onClick={() => handleFilterClick("pricerange")}
            >
              Price Range
            </div>
            <div
              className={`filter-item ${selectedFilter === "category" ? "active" : ""}`}
              onClick={() => handleFilterClick("category")}
            >
              Category
            </div>
            <div
              className={`filter-item ${selectedFilter === "discount" ? "active" : ""}`}
        ...
        <div className="myfilter-option">
                ...
                {distinctcat.map((category, id) => (
                  <li key={id} className="category-item" style={{ borderBottom: "1px solid white" }}>
                    ...
                    >
                      {tags.map((subCategory, id) => (
                        <li key={id} className="subcategory-item" style={{ display: "flex", justifyContent: "space-between" }}>
                          <div onClick={() => handleCategoryChange(subCategory)}>{subCategory}</div>
                          <input type="radio" onClick={() => handleCategoryChange(subCategory)} />
                        </li>
                      ))}
                    </ul>
          ...
          {selectedFilter === "discount" && (
            <div className="filter-value occasionss">
              <div className="color-options">
                {["40% and above", "30% and above", "20% and above", "10% and above"].map(
                  (discount) => (
                    <div
                      key={discount}
                      style={{
                    ...
                    >
                      <p className="nackline-btn">{discount}</p>
                      <input
                        type="checkbox"
                        className={`occasion-btn ${
                          filters.discount.includes(discount) ? "active" : ""
                        }`}
