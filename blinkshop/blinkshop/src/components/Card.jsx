  ...
  console.log("sv",searchvalue)
    ...
    <AiOutlineDelete onClick={() => removewishlistonly(product.itemid)} style={{ color: "black", position: 'relative', left: "-3px", bottom: "2px" }} size={15} />
        ...
        <div className="product-details">
          <p className="product-title">{product.description.length>19?(product.description.slice(0,34)+`...`):(product.description)}</p>
        ...
        {!wish?(<div className="delivery-info" style={{fontSize:'.7rem',marginBottom:'20px'}}>⚡Delivery in 60 min</div>):( <button className="delivery-info" style={{paddingBottom:"10px",textAlign:'center'}}  onClick={()=>{setShowSize(product.itemid)}}>Add to Cart</button>)} 
        </div>
      </div>
            ))
          ) : (
