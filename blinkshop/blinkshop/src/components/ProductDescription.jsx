...
const targetRef = useRef(null);
  ...
  // 🔑 FIX: gallery now sources from product.image (product-level images) first.
  // Falls back to sizes[0].image only for older products that don't have
  // product-level images yet, since size images are now optional.
  const galleryImages =
    ...
    if (data.productdetails && data.productdetails.length > 0) {
      ...
      if (defaultVariant) {
        setproduct({
          ...
          printtype: mainProduct.printtype,
          ratingCount: mainProduct.ratingCount,
          // 🔑 FIX: keep the FULL image array here (don't collapse to [0])
          // so the gallery can show every product image, not just one.
      ...
      setproduct({
        ...
        occasion: variantProduct.occasion,
        printtype: variantProduct.printtype,
        // 🔑 FIX: same here — keep the full array
...
useEffect(() => {
  if (product?.cate && product?.tag) {
    const fetchAndSetCoupons = async () => {
      try {
        setIsLoading(true);
        await fetchCoupons(false, product.cate, product.tag);
      } catch (err) {
        console.error("Error fetching coupons:", err);
      } finally {
        setIsLoading(false);
 ...
 useEffect(() => {
  ...
  if (window.fbq && product) {
    window.fbq("track", "ViewContent", {
      ...
      title: product.title,
      description: product.description,
      // 🔑 FIX: pull first image from the new gallery source
...
const handleNext = () => {
  ...
  const handleTouchEnd = () => {
    ...
    if (distance > threshold && currentImageIndex < galleryImages.length - 1) {
      setCurrentImageIndex(currentImageIndex + 1);
    }
  ...
  const handlePopupTouchEnd = () => {
    ...
    if (distance > threshold && popupCurrentIndex < galleryImages.length - 1) {
      setPopupCurrentIndex(popupCurrentIndex + 1);
    }
      ...
      <div className="product-page">
        ...
        <div className="image-slider">
          ...
          {galleryImages.map((image, index) => (
            <div key={index} className="image-slide">
              <img
                src={cloudinaryImg(galleryImages[currentImageIndex], 1000)}
                alt={product.title || "dress"}
                onClick={() => handleImageClick(image, currentImageIndex)}
                onTouchStart={handleTouchStart}
                onTouchMove={handleTouchMove}
      ...
      {isPopupOpen && (
        <div className="popup"  style={{backgroundColor:"black"}}>
          <div className="image-slider">
            <div className="image-slide" style={{minHeight:"100vh"}}>
              <img
                src={cloudinaryImg(galleryImages[popupCurrentIndex], 1000)}
                alt={`Product ${popupCurrentIndex + 1}`}
                onTouchStart={handlePopupTouchStart}
                onTouchMove={handlePopupTouchMove}
                onTouchEnd={handlePopupTouchEnd}
