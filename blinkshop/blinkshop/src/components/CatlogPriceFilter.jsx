...
const CatlogPriceFilter = () => {
  ...
  const initialMaxPrice = 3000;
  const inimindis=2
  const inimaxdis=50
  const { wearsdata,filters,setFilters, handleapplymyfilters } = useBio();
  ...
  const slideMin = (e) => {
    ...
    if (value >= sliderMinValue && maxVal - value >= minGap) {
      setMinVal(value);
      setMinInput(value);
      setFilters({ ...filters, pricerangemin: e.target.value});
    }
  };
  ...
  const slideMax = (e) => {
    ...
    if (value <= sliderMaxValue && value - minVal >= minGap) {
      setMaxVal(value);
      setMaxInput(value);
      setFilters({ ...filters, pricerangemax: e.target.value });
    }
  };
    ...
    <div className="double-slider-box">
      ...
      <div className="range-slider">
        ...
        <input
          ...
          onTouchEnd={stopDrag}
          className="min-vall"
                            style={{border:"none",margin:"5px",padding:"0"}}
        ...
        <input
          ...
          onTouchEnd={stopDrag}
          className="max-vall"
          style={{border:"none",margin:"5px",padding:"10px"}}
        />
