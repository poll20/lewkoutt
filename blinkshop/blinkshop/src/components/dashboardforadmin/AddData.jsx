...
//   // accidentally be a size-image input instead of the product-images input once colors
//   // were added (since size inputs are also type="file" multiple).
...
//   // Size images are uploaded here, ONCE, immediately. size.image ends up
//   // holding real ImageKit URLs — handleSubmit must never touch these again.
//   const addSize = async (colorIndex, isNewCategory) => {
//     const targetForm = isNewCategory ? newProduct : product;
//     const updatedColors = [...targetForm.productdetails.colors];
...
//       size: newSize.size,
//       quantity: newSize.quantity,
//       image: uploadedUrls, // already final ImageKit URLs
//     };
...
//   // could match the wrong <input>. Both are corrected below.
//   const handleSubmit = async (e) => {
//     e.preventDefault();
...
//       // 1. Product gallery images — raw File objects, uploaded for the first time here.
//       const productFiles = productImagesInputRef.current
//         ? Array.from(productImagesInputRef.current.files)
//         : [];
//       const uploadedProductImages = productFiles.length
//         ? await uploadToImageKitMultiple(productFiles)
//         : [];

//       // 2. Color images — raw File objects (if any were attached), uploaded here.
//       const colorImages = targetProduct.productdetails.colors
//         .map((c) => c.image)
//         .filter(Boolean);
//       const uploadedColorImages = colorImages.length
//         ? await uploadToImageKitMultiple(colorImages)
//         : [];

//       // 3. Size images — DO NOT re-upload. They were already uploaded to
//       // ImageKit inside addSize() and color.sizes already holds final URLs.
//       const updatedProduct = {
//         ...targetProduct,
//         productdetails: {
//           ...targetProduct.productdetails,
...
//             ...color,
//             image: uploadedColorImages[i] || "",
//             sizes: color.sizes, // already final URLs — passed through untouched
//           })),
//         },
//       };
...
//           <div style={{ animation: 'fadeIn 0.5s ease-out' }}>
//             <div style={{ textAlign: 'center', marginBottom: '32px' }}>
//               <h2 style={{ fontSize: '2rem', fontWeight: '700', color: '#1f2937', marginBottom: '8px' }}>
//                 Choose Category
//               </h2>
//               <p style={{ color: '#6b7280', fontSize: '1.1rem' }}>
//                 Select the category where you want to add your product
...
//                               productdetails: {
//                                 ...prev.productdetails,
//                                 image: [...prev.productdetails.image, ...files],
//                               },
//                             }))
//                           : setProduct((prev) => ({
//                               ...prev,
...
//                           background: color.color.toLowerCase(), border: '2px solid #e5e7eb',
//                         }}></div>
//                         {color.color}
//                       </h4>
...
const AddData = () => {
  ...
  const addSize = async (colorIndex, isNewCategory) => {
    ...
    // 🔑 Image is now optional — size/quantity are still required.
    if (!newSize.size || !newSize.quantity) {
      alert("Please fill size and quantity.");
      return;
    }

    // Only call uploadToImageKitMultiple if the admin actually selected files.
    // No files selected → image stays an empty array, no upload call is made.
    const uploadedUrls = files.length > 0
      ? await uploadToImageKitMultiple(files)
      : [];

    const sizeData = {
      size: newSize.size,
      quantity: newSize.quantity,
      image: uploadedUrls, // [] if admin left it empty, otherwise final ImageKit URLs
    };
                ...
                <div style={{ marginTop: '24px' }}>
                  {(step === 1 ? newProduct.productdetails.colors : product.productdetails.colors).map((color, index) => (
                    <div key={index} className="color-card">
                      ...
                      <div>
                        <label style={{ ...labelStyle, fontSize: '0.8rem' }}>Size Images (optional)</label>
                        <input
                          type="file"
                          multiple
                          onChange={handleSizeImageUpload}
